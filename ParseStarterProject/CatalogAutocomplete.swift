...
@objc protocol MPGTextFieldDelegateCatalog{
    func dataForPopoverInTextField(textfield: Catalog_Autocomplete) -> [Dictionary<String, AnyObject>]
    
    optional func textFieldDidEndEditing(textField: Catalog_Autocomplete, withSelection data: Dictionary<String,AnyObject>)
    optional func textFieldShouldSelect(textField: Catalog_Autocomplete) -> Bool
}

class Catalog_Autocomplete: UITextField, UITextFieldDelegate, UITableViewDelegate, UITableViewDataSource, UIGestureRecognizerDelegate {
    
    var mDelegate : MPGTextFieldDelegateCatalog?
    ...
    */
    ...
    func provideSuggestions(){
        if let tvc = self.tableViewController {
            ...
            var frameForPresentation = self.superview!.frame//self.frame//self.superview!.frame
            frameForPresentation.origin.y += self.superview!.frame.size.height;//self.frame.size.height;
            frameForPresentation.size.height = 200;
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        let displayImage : AnyObject? = dataForRowAtIndexPath["DisplayImage"]
        cell!.textLabel!.text = displayText as? String
        cell!.textLabel!.font = UIFont(name:"Avenir-Book", size:17)
        cell!.textLabel!.textColor = UIColorFromHex(0x898989)//2A2F36)
        //cell!.detailTextLabel!.text = displaySubText as? String
        cell!.imageView?.image = displayImage as? UIImage
